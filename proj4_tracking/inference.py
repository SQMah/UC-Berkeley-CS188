...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            self.beliefs[pos] = self.getObservationProb(observation, pacman_pos, pos, jail_pos) * self.beliefs[pos]
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        """
        pos_dists = {pos: self.getPositionDistribution(gameState, pos) for pos in self.allPositions}
        old_beliefs = self.beliefs.copy()
        for new_pos in self.allPositions:
            sum_pos = 0
            for pos in self.allPositions:
                sum_pos += pos_dists[pos][new_pos] * old_beliefs[pos]
            self.beliefs[new_pos] = sum_pos
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            self.particles.extend([pos] * num_per_pos)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            weight = self.getObservationProb(observation, pacman_pos, p, jail_pos)
            dist[p] += weight
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                weight *= self.getObservationProb(observation[i], pacman_pos, p[i], jail_pos)
            dist[p] += weight
        if dist.total() == 0:
            self.initializeUniformly(gameState)
        else:
